...
(function (){
  ...
  Scripts.prototype.collectFromDisk=function(){
    ...
    // NEW: skip folders we never want to scan into
    function isIgnoredFolder(name){
      return name.charAt(0)=="." || name=="node_modules" || name=="__tests__" || name=="tests";
    }

    // NEW: walk a folder and every subfolder beneath it, however deep,
    // collecting scripts under a path-like label so each subfolder still
    // shows up as its own group in the panel
    function collectRecursive(folder, label){
      var scripts=scriptsIn(folder);
      if(scripts)
        that.collection[label]=scripts;
      var subs=foldersIn(folder);
      for(var j=0;j<subs.length;j++){
        var sub=subs[j];
        var subName=decodeURI(sub.name);
        if(isIgnoredFolder(subName)) continue;
        collectRecursive(sub, label+"/"+subName);
      }
    }
    ...
    //extract scripts from all folders, now recursing into subfolders too
    var l = folders.length;
    for (var i = 0; i < l; i++) {
      folder = folders[i];
      collectRecursive(folder, decodeURI(folder.name));
    }
  }
