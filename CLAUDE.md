...
- Claudeが作業する際は `claude/〇〇` という作業ブランチが自動的に作られる。
  **mainには直接コミットしない。**
- 作業が終わったらPull Requestを作成し、ユーザーがGitHub上で内容を確認して
  Mergeする、という流れが正規のフロー。
- リポジトリ設定で「Automatically delete head branches」をONにしてあるため、
  PRがマージされると作業ブランチは自動的に削除される。ユーザー側で手動削除
  する必要は基本的にない。
- 例外的に古い作業ブランチが残ってしまっていた場合は、削除前に
  `git compare main...<branch>`（またはGitHub上のCompareページ）で
  `ahead_by` が0であること（＝中身が既にmainに含まれていること）を確認して
  から削除する。
